...
import json
import logging
from logging.handlers import TimedRotatingFileHandler # for creating self-managing log files
  ...
  for line in user_db_lines:
    ...
    if (len(fields) == 3):
      for i in range(0,len(fields)):
        fields[i] = ' '.join(fields[i].split()) # clean up multiple spaces
...
def auth_respond(error=0,message='',user_name='',tool_auths=''):
  global logger
  logger.info('error:' + str(error) + ', message:' + message + ', user_name:' + user_name + ', tool_auths:' + tool_auths)
...
def getauth_from_db(args):
  """
  looks up card id in DB and determines if access should be granted
...
def setup_logger():
    ...
    loghandler = TimedRotatingFileHandler(log_file_path + '/log', when="W0", interval=1, backupCount=52)
...
setup_logger()
api.add_resource(getauth_rest  , '/novapass/api/v1.0/getauth', endpoint='getauth')
api.add_resource(log_rest      , '/novapass/api/v1.0/log'    , endpoint='log')
